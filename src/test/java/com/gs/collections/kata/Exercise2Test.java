...
public class Exercise2Test extends CompanyDomainForKata
{
     ...
     */
//    private static final Predicate<Customer> CUSTOMER_FROM_LONDON = Predicates.attributeEqual(Customer.TO_CITY, ("London"));
...
//    private static final Predicate<Customer> CUSTOMER_FROM_LONDON = new Predicates<Customer>() {
//        @Override
//        public boolean accept(Customer each) {
//            return each.getCity().equals("London");
//        }
//    };
    ...
    {
        long size = this.company.getCustomers().stream().filter(customer -> customer.getCity().equals("London")).count();
//        List<Customer> cus = null;
//        cus.stream().filter(customer -> customer.getCity().equals("London")).collect(Collectors.toList());
        int numberOfCustomerFromLondon = this.company.getCustomers().count(CUSTOMER_FROM_LONDON);
        Assert.assertEquals("Should be 2 London customers", 2, size);
    }
    ...
    {
        MutableList<Customer> customersFromLondon = this.company.getCustomers().partition(CUSTOMER_FROM_LONDON).getSelected();
        MutableList<Customer> customersNotFromLondon = this.company.getCustomers().partition(CUSTOMER_FROM_LONDON).getRejected();
        Verify.assertSize("Should be 2 London customers", 2, customersFromLondon);
        Verify.assertSize("customers not from London", 1, customersNotFromLondon);
    }
